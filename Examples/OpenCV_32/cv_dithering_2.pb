...
#CV_DESCRIPTION = "Implementation of the Floyd-Steinberg dithering algorithm applied to a color image." +
  ...
  If FileSize(ImageFile) > 0
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200 And *resize\nChannels = 3
      ...
      *dither.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_8U, 3)
      scalar.CvScalar : scalar1.CvScalar : scalar2.CvScalar : scalar3.CvScalar : scalar4.CvScalar

      For y = 0 To *resize\height - 2
        For x = 1 To *resize\width - 2
          cvGet2D(@scalar, *resize, y, x)
          valIn = (scalar\val[0] + scalar\val[1] + scalar\val[2]) / 3

          If valIn < 128 : valOut = 0 : Else : valOut = 255 : EndIf

          cvSet2D(*dither, y, x, valOut, valOut, valOut, valOut)
          diffValue = valIn - valOut
          cvGet2D(@scalar1, *resize, y, x + 1)
          cvGet2D(@scalar2, *resize, y + 1, x - 1)
          cvGet2D(@scalar3, *resize, y + 1, x)
          cvGet2D(@scalar4, *resize, y + 1, x + 1)
          diff1 = diffValue * 7 / 16
          diff2 = diffValue * 3 / 16
          diff3 = diffValue * 5 / 16
          diff4 = diffValue * 1 / 16
          ...
          For z = 0 To 3 - 1
            ...
            scalar4\val[z] + diff4
          Next
          cvSet2D(*resize, y, x + 1, scalar1\val[0], scalar1\val[1], scalar1\val[2], scalar1\val[3])
          cvSet2D(*resize, y + 1, x - 1, scalar2\val[0], scalar2\val[1], scalar2\val[2], scalar2\val[3])
          cvSet2D(*resize, y + 1, x, scalar3\val[0], scalar3\val[1], scalar3\val[2], scalar3\val[3])
          cvSet2D(*resize, y + 1, x + 1, scalar4\val[0], scalar4\val[1], scalar4\val[2], scalar4\val[3])
        Next
      Next
