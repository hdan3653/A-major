...
Global *save.IplImage, exitCV.b, lpPrevWndFunc, nDepth

#CV_WINDOW_NAME = "PureBasic Interface to OpenCV"
#CV_DESCRIPTION = "Using a second image as a mask to compare against an object in the original image, " +
                  "depth of field is simulated." + Chr(10) + Chr(10) +
                  "- SPACEBAR: Start the simulation." + Chr(10) + Chr(10) +
...
ProcedureC DepthFilter(*depth.CvMat, *horiz.CvMat, radius.f)
  myinf.d = 9.9e307
  h = *depth\rows
  w = *depth\cols
  channel = (CV_MAT_TYPE(*depth\type) / 8) + 1
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      cvGet2D(@scalar1, *final, i, j)
	    cvSet2D(*depth, i, j, scalar1\val[0], scalar1\val[1], scalar1\val[2], 0)
    Next
  Next
  cvReleaseMat(@*flag)
  cvReleaseMat(@*b)
...
ProcedureC DepthOfField(*image.IplImage, sigma_s.f, sigma_r.f, *mask.IplImage)
  iterations = 3
  w = *mask\width
  h = *mask\height
	channel = *mask\nChannels
  ...
  *ctV.CvMat = cvCreateMat(h, w, CV_MAKETYPE(#CV_32F, 1))
  ...
  For j = 0 To w - 1
    ...
    Next
  Next
  *depth.CvMat = cvCreateMat(h, w, CV_MAKETYPE(#CV_32F, 3))
  *depth_t.CvMat = cvCreateMat(w, h, CV_MAKETYPE(#CV_32F, 3))
	...
	For i = 0 To h - 1
	  For j = 0 To w - 1
	    ...
	    cvSet2D(*depth, i, j, val0, val1, val2, 0)
    Next
  Next
  *ctV_t.CvMat = cvCreateMat(w, h, CV_MAKETYPE(#CV_32F, 1))
	...
	For i = 0 To iterations - 1
	  ...
	  DepthFilter(*depth, *ctH, radius)
	  cvTranspose(*depth, *depth_t)
	  DepthFilter(*depth_t, *ctV_t, radius)
	  cvTranspose(*depth_t, *depth)
	Next
	*final.CvMat = cvCreateMat(h, w, CV_MAKETYPE(#CV_32F, 3))
	scalar.CvScalar
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      cvGet2D(@scalar, *depth, i, j)
      cvSet2D(*final, i, j, scalar\val[0], scalar\val[1], scalar\val[2], 0)
    Next
  Next
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      For c = 0 To channel - 1
        If PeekF(@CV_IMAGE_ELEM(*mask, i, (j * channel + c) * 4)) > 0.5
          PokeF(@CV_IMAGE_ELEM(*mask, i, (j * channel + c) * 4), 1)
        EndIf
      Next
    Next
  Next
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      For c = 0 To channel - 1
        If PeekF(@CV_IMAGE_ELEM(*mask, i, (j * channel + c) * 4)) = 1
          PokeF(@*final\fl\f + i * *final\Step + (j * channel + c) * 4, PeekF(@CV_IMAGE_ELEM(*image, i, (j * channel + c) * 4)))
        EndIf
      Next
    Next
  Next
  cvReleaseMat(@*ctV_t)
  cvReleaseMat(@*depth_t)
  cvReleaseMat(@*depth)
  cvReleaseMat(@*ctV)
  cvReleaseMat(@*ctH)
  cvReleaseMat(@*vert)
  ...
  If FileSize(ImageFile) > 0
    ...
    *param\uValue = window_handle
    ...
    If keyPressed = 32
      *resize32.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_32F, 3)
      ...
      cvWaitKey(100)
  		sigma_s.f = 20
  		sigma_r.f = 0.8
  		*mask.IplImage = cvLoadImage("images/depth" + Str(nDepth + 1) + "b.jpg", #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)

  		If *mask\nChannels = 1
    		*color.IplImage = cvCreateImage(*mask\width, *mask\height, #IPL_DEPTH_8U, 3)
    		cvCvtColor(*mask, *color, #CV_GRAY2BGR, 1)
    	Else
    	  *color.IplImage = cvCloneImage(*mask)
    	EndIf
  		*mask32.IplImage = cvCreateImage(*mask\width, *mask\height, #IPL_DEPTH_32F, 3)
  		cvConvertScale(*color, *mask32, 1 / 255, 0)
  		*depth.CvMat = DepthOfField(*resize32, sigma_s, sigma_r, *mask32)
  		temp.IplImage
  		*temp.IplImage = cvGetImage(*depth, @temp)
    ...
    If keyPressed = 13
      exitCV = #False
      nDepth = (nDepth + 1) % 2
      OpenCV("images/depth" + Str(nDepth + 1) + "a.jpg")
    EndIf
  EndIf
EndProcedure

ExamineDesktops()
OpenCV("images/depth1a.jpg")
