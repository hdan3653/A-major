...
Global *save.IplImage, exitCV.b, lpPrevWndFunc, nBlend

#CV_WINDOW_NAME = "PureBasic Interface to OpenCV"
#CV_DESCRIPTION = "Using a Poisson blending algorithm, a localized color change is achieved." + Chr(10) + Chr(10) +
                  "- SPACEBAR: Execute recoloring." + Chr(10) + Chr(10) +
...
ProcedureC PoissonBlend(*image.IplImage, *mask.IplImage, posX, posY)
  ...
  *factor.IplImage = cvCreateImage(w, h, #IPL_DEPTH_32F, 3)
  cvConvertScale(*res, *factor, 1 / 255, 0)
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      PokeF(@CV_IMAGE_ELEM(*factor, i, (j * channel + 0) * 4), nB)
      PokeF(@CV_IMAGE_ELEM(*factor, i, (j * channel + 1) * 4), nG)
      PokeF(@CV_IMAGE_ELEM(*factor, i, (j * channel + 2) * 4), nR)
    Next
  Next
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      For c = 0 To channel - 1
        PokeF(@CV_IMAGE_ELEM(*srx_32, i, (j * channel + c) * 4), PeekF(@CV_IMAGE_ELEM(*srx32, i, (j * channel + c) * 4)) * PeekF(@CV_IMAGE_ELEM(*factor, i, (j * channel + c) * 4)))
        PokeF(@CV_IMAGE_ELEM(*sry_32, i, (j * channel + c) * 4), PeekF(@CV_IMAGE_ELEM(*sry32, i, (j * channel + c) * 4)) * PeekF(@CV_IMAGE_ELEM(*factor, i, (j * channel + c) * 4)))
      Next
    Next
  Next
  cvNot(*ero, *ero)
...
ProcedureC OpenCV(ImageFile.s)
  If FileSize(ImageFile) > 0
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    Select nBlend
      Case 0
        *roi.IplImage = cvCreateImage(*resize\width - 45, *resize\height - 50, *resize\depth, *resize\nChannels)
        x = 20
        y = 20
        cvSetImageROI(*resize, x, y, *resize\width - 45, *resize\height - 50)
      Case 1
        x = 40
        y = 10
        *roi.IplImage = cvCreateImage(*resize\width - x * 2 - 5, *resize\height - y * 2, *resize\depth, *resize\nChannels)
        cvSetImageROI(*resize, x, y, *resize\width - x * 2 - 5, *resize\height - y * 2)
    EndSelect
    ...
    If *resize\nChannels = 3
      ...
      *blend.IplImage = PoissonBlend(*resize, *roi, x, y)
      ...
      FreeMemory(*param)
    Else
      MessageBox_(0, "Process Cancelled..." + Chr(10) + Chr(10) + "Image does not meet the channel requirements.", #CV_WINDOW_NAME, #MB_ICONERROR)
    EndIf
    cvReleaseImage(@*roi)
    ...
    If keyPressed = 13
      exitCV = #False
      nBlend = (nBlend + 1) % 2
      OpenCV("images/blend4_" + Str(nBlend + 1) + ".jpg")
    EndIf
  EndIf
EndProcedure

ExamineDesktops()
OpenCV("images/blend4_1.jpg")
