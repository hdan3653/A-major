...
#CV_DESCRIPTION = "Finds edges in an image using the Canny86 algorithm." + Chr(10) + Chr(10) +
                  "- TRACKBAR: Adjust the threshold." + Chr(10) + Chr(10) +
                  "- SPACEBAR: Toggle color ON / OFF."
  ...
  If FileSize(ImageFile) > 0
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *image\width >= dtWidth - 100 Or *image\height >= dtHeight - (100 + 48)
      ...
      *resize.IplImage = cvCreateImage(iWidth, iHeight, #IPL_DEPTH_8U, 3)
    ...
    If *resize\width > 200 And *resize\height > 200 And *resize\nChannels = 3
      cvCreateTrackbar("Threshold", #CV_WINDOW_NAME, 0, 100, @CvTrackbarCallback())
      ...
      *canny.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_8U, 3)
      cvCvtColor(*resize, *gray, #CV_BGR2GRAY, 1)
      cvSmooth(*gray, *edge, #CV_BLUR, 3, 3, 0, 0)
      cvNot(*gray, *edge)
      *param.USER_INFO = AllocateMemory(SizeOf(USER_INFO))
      *param\uPointer1 = *canny
      *param\uValue = window_handle
      cvSetMouseCallback(*window_name, @CvMouseCallback(), *param)

      Repeat
        If *canny
          cvCanny(*gray, *edge, nThreshold, nThreshold * 3, 3, #False)
          cvSetZero(*canny)

          If canny : cvCvtColor(*edge, *canny, #CV_GRAY2BGR, 1) : Else : cvCopy(*resize, *canny, *edge) : EndIf

          cvShowImage(#CV_WINDOW_NAME, *canny)
          keyPressed = cvWaitKey(0)

          If keyPressed = 32 : canny ! 1 : EndIf
