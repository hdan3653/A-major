...
#CV_DESCRIPTION = "Calculating the back projection of a histogram, frequent density values are displayed as bright pixels." +
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200 And *resize\nChannels = 3
      *gray.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_8U, 1)
      ...
      Dim *projection.IplImage(2) : *projection(0) = cvCloneImage(*gray) : *projection(1) = cvCloneImage(*gray)
      Dim bins(2) : bins(0) = 32 : bins(1) = 256
      Dim range1.f(2) : range1(0) = 0 : range1(1) = 256
      Dim range2.f(2) : range2(0) = 0 : range2(1) = 256
      Dim *ranges(2) : PokeL(@*ranges(0), @range1()) : PokeL(@*ranges(1), @range2())
      *hist.CvHistogram = cvCreateHist(2, @bins(), #CV_HIST_ARRAY, @*ranges(), 1)
      cvCalcHist(*projection(), *hist, #False, #Null)
      cvGetMinMaxHistValue(*hist, #Null, @max_value.f, #Null, #Null)
      cvNormalizeHist(*hist, *resize\width * *resize\height * 255 / max_value)
      cvCalcBackProject(*projection(), *gray, *hist)
      ...
      *param\uValue = window_handle
      ...
      cvReleaseImage(@*color)
      cvReleaseHist(@*hist)
      cvReleaseImage(@*projection(1))
      cvReleaseImage(@*projection(0))
      cvReleaseImage(@*gray)
