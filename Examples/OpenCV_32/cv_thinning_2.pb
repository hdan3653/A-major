...
#CV_DESCRIPTION = "Repeated iterations using multiple filters on a grayscale image producing a skeleton effect." +
                  Chr(10) + Chr(10) + "- SPACEBAR: Start skeleton process." +
    ...
    If *resize\width > 200 And *resize\height > 200
      ...
      *param\uValue = window_handle
      ...
      If keyPressed = 32
        ...
        *erode.CvMat = cvCreateMat(*resize\height, *resize\width, CV_MAKETYPE(#CV_8U, 1))
        *dilate.CvMat = cvCreateMat(*resize\height, *resize\width, CV_MAKETYPE(#CV_8U, 1))
        *thin.CvMat = cvCreateMat(*resize\height, *resize\width, CV_MAKETYPE(#CV_8U, 1))
        *kernel.IplConvKernel = cvCreateStructuringElementEx(3, 3, 1, 1, #CV_SHAPE_CROSS, #Null)
        cvThreshold(*resize, *resize, 15, 255, #CV_THRESH_BINARY)
        cvSetZero(*thin)

        Repeat
          cvErode(*resize, *erode, *kernel, 1)
          cvDilate(*erode, *dilate, *kernel, 2)
          cvSub(*resize, *dilate, *dilate, #Null)
          cvOr(*thin, *dilate, *thin, #Null)
          cvCopy(*erode, *resize, #Null)
          cvShowImage(#CV_WINDOW_NAME, *thin)
          keyPressed = cvWaitKey(10)
        Until cvCountNonZero(*resize) = 0 Or keyPressed = 13 Or keyPressed = 27 Or exitCV
        ...
        If keyPressed <> 13 And keyPressed <> 27 And exitCV = #False
          ...
          *reset2.IplImage = cvCloneImage(*color)
          ...
          *temp.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_8U, 3)
          cvCvtColor(*reset1, *temp, #CV_GRAY2BGR, 1)
