...
Global getCV.b, *save.IplImage, exitCV.b, lpPrevWndFunc, nCorner.d

#CV_WINDOW_NAME = "PureBasic Interface to OpenCV"
#CV_DESCRIPTION = "Calculates a feature map for corner detection using the Harris edge detector." + Chr(10) + Chr(10) +
                  "- TRACKBAR: Adjust the detection."
  ...
  If FileSize(ImageFile) > 0
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200
      cvCreateTrackbar("Detection", #CV_WINDOW_NAME, 0, 5, @CvTrackbarCallback())
      ...
      EndIf
      *harris.IplImage
      *map.CvMat = cvCreateMat(*resize\height, *resize\width, CV_MAKETYPE(#CV_32F, 1))
      cvCornerHarris(*gray, *map, 3, 3, 0.04)
      harris.CvScalar
      nCorner = 0.01
      *param.USER_INFO = AllocateMemory(SizeOf(USER_INFO))
      *param\uPointer1 = *harris
      *param\uValue = window_handle
      cvSetMouseCallback(*window_name, @CvMouseCallback(), *param)

      Repeat
        If *resize
          *harris = cvCloneImage(*resize)

          For y = 0 To *resize\height - 1
            For x = 0 To *resize\width - 1
              cvGet2D(@harris, *map, y, x)

              If harris\val[0] > nCorner : cvCircle(*harris, x, y, 2, 0, 0, 255, 0, #CV_FILLED, #CV_AA, #Null) : EndIf

            Next
          Next
          cvShowImage(#CV_WINDOW_NAME, *harris)
