...
#CV_DESCRIPTION = "Raises every array element of an image to a power." + Chr(10) + Chr(10) +
                  "- SPACEBAR: Change Power value." + Chr(10) + Chr(10) +
  ...
  If FileSize(ImageFile) > 0
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200 And *resize\nChannels = 3
      *pow.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_8U, 3)
      cvPow(*resize, *pow, 2)
      *reset.IplImage = cvCloneImage(*pow)
      ...
      *param\uValue = window_handle
      ...
      Repeat
        If *pow
          Select PIP
            Case 0
              cvSetImageROI(*pow, 20, 20, iWidth, iHeight)
              cvAndS(*pow, 0, 0, 0, 0, *pow, #Null)
              cvAdd(*pow, *PIP, *pow, #Null)
              cvResetImageROI(*pow)
              cvRectangleR(*pow, 19, 19, iWidth + 2, iHeight + 2, 0, 255, 255, 0, 1, #CV_AA, #Null)
            Case 1
              cvSetImageROI(*pow, *pow\width - (150 + 20), 20, iWidth, iHeight)
              cvAndS(*pow, 0, 0, 0, 0, *pow, #Null)
              cvAdd(*pow, *PIP, *pow, #Null)
              cvResetImageROI(*pow)
              cvRectangleR(*pow, *pow\width - (150 + 21), 19, iWidth + 2, iHeight + 2, 0, 255, 255, 0, 1, #CV_AA, #Null)
          EndSelect
          cvShowImage(#CV_WINDOW_NAME, *pow)
          ...
          Select keypressed
            Case 32
              ...
              *reset = cvCloneImage(*pow)
            Case 86, 118
              PIP = (PIP + 1) % 3
              cvReleaseImage(@*pow)
              *pow = cvCloneImage(*reset)
              *param\uPointer1 = *pow
          EndSelect
        EndIf
