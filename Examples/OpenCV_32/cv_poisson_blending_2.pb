...
#CV_DESCRIPTION = "Using a mixed Poisson blending algorithm, two images are blended together." + Chr(10) + Chr(10) +
                  "- SPACEBAR: Add image / Blend." + Chr(10) + Chr(10) +
  ...
  For i = posX To posX + wMask - 1
    ...
    For j = 0 To hMask - 2
      For c = 0 To channel1 - 1
       ...
       If Abs(PeekF(@CV_IMAGE_ELEM(*sgx, j, (ii * channel1 + c) * 4)) - PeekF(@CV_IMAGE_ELEM(*sgy, j, (ii * channel1 + c) * 4))) > Abs(PeekF(@CV_IMAGE_ELEM(*grx, jj, (i * channel1 + c) * 4)) - PeekF(@CV_IMAGE_ELEM(*gry, j, (i * channel1 + c) * 4)))
       ...
       Else
          PokeF(@CV_IMAGE_ELEM(*bmaskx, jj, (i * channel1 + c) * 4), PeekF(@CV_IMAGE_ELEM(*grx, jj, (i * channel1 + c) * 4)))
          PokeF(@CV_IMAGE_ELEM(*bmasky, jj, (i * channel1 + c) * 4), PeekF(@CV_IMAGE_ELEM(*gry, jj, (i * channel1 + c) * 4)))
        EndIf
      Next
      jj + 1
    Next
...
ProcedureC OpenCV(ImageFile.s, nBlend)
  If FileSize(ImageFile) > 0
    ...
    *roi.IplImage = cvLoadImage("images/blend2_" + Str(nBlend + 1) + "b.jpg", #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
      ...
      *param\uValue = window_handle
      ...
      Select nBlend
        ...
        Case 1
          ...
          y = 10
        Case 2
          x = (*resize\width / 2) - (*roi\width / 2) - 60
          y = *resize\height - *roi\height - 30
        Case 3
          x = *resize\width - *roi\width
          y = 4
        Case 4
          x = *resize\width - *roi\width - 25
          y = 25
      EndSelect
    ...
    If keyPressed = 13
      exitCV = #False
      nBlend = (nBlend + 1) % 5
      OpenCV("images/blend2_" + Str(nBlend + 1) + "a.jpg", nBlend)
    EndIf
  EndIf
EndProcedure

ExamineDesktops()
OpenCV("images/blend2_1a.jpg", 0)
