...
#CV_DESCRIPTION = "CAPTCHA: Completely Automated Public Turing test to tell Computers and Humans Apart." + Chr(10) + Chr(10) +
                  "SPACEBAR: Generate new CAPTCHA."
...
ProcedureC Transform(*image.CvMat)
...
ProcedureC Rotate(*input.CvMat, *output.CvMat)
  sign = Random(32767) % 2
...
ProcedureC Scale(*input.CvMat, height.f, width.f)
  h = Random(32767) % 20 * -1
  w = Random(32767) % 20 * -1
  height = h + height
  width = w + width
...
ProcedureC AddLines(*image.IplImage)
  For rtnCount = 0 To #CAPTCHA_LENGTH
    ...
    cvLine(*image, startX, startY, endX, endY, Random(255), Random(255), Random(255), 0, Random(3), #CV_AA, #Null)
  Next
EndProcedure

ProcedureC AddNoise(*image.IplImage)
  ...
  If *resize
    ...
    If keyPressed = 32
      ...
      For rtnCount = 0 To #CAPTCHA_LENGTH - 1
        ...
        Transform(*char)
        Rotate(*char, *char)
        *scale = Scale(*char, #CHAR_HEIGHT, #CHAR_WIDTH)
