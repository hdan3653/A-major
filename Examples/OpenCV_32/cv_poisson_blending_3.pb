...
#CV_DESCRIPTION = "Using a Poisson blending algorithm, a monochrome transfer is achieved between two images." +
                  Chr(10) + Chr(10) + "- SPACEBAR: Add image / Blend." +
                  Chr(10) + Chr(10) + "- ENTER: Switch to next image." +
                  Chr(10) + Chr(10) +"Double-Click the window to open a link to more information."
...
ProcedureC ShowImage(*target.IplImage, *source.IplImage, x, y)
  scalar.CvScalar

  For xx = 0 To *source\width - 1
    For yy = 0 To *source\height - 1
      cvGet2D(@scalar, *source, yy, xx)
      cvSet2D(*target, yy + y, xx + x, scalar\val[0], scalar\val[1], scalar\val[2], 0)
    Next
  Next
EndProcedure

ProcedureC GetGradientX(*image.IplImage, *gx.IplImage)
  w = *image\width
  h = *image\height
  channel = *image\nChannels
  cvSetZero(*gx)
...
ProcedureC GetGradientY(*image.IplImage, *gy.IplImage)
  w = *image\width
  h = *image\height
  channel = *image\nChannels
  cvSetZero(*gy)
...
ProcedureC LapX(*image.IplImage, *gxx.IplImage)
  w = *image\width
  h = *image\height
  channel = *image\nChannels
  cvSetZero(*gxx)
...
ProcedureC LapY(*image.IplImage, *gyy.IplImage)
  w = *image\width
  h = *image\height
  channel = *image\nChannels
  cvSetZero(*gyy)
...
ProcedureC Dst(Array gtest.d(1), Array gfinal.d(1), h, w)
...
ProcedureC iDst(Array gtest.d(1), Array gfinal.d(1), h, w)
  nn = h + 1
...
ProcedureC Transpose(Array mat.d(1), Array mat_t.d(1), h, w)
...
ProcedureC PoissonSolver(*image.IplImage, *gxx.IplImage, *gyy.IplImage, *result.CvMat)
  w = *image\width
  h = *image\height
  channel = *image\nChannels
...
ProcedureC PoissonBlend(*image.IplImage, *mask.IplImage, posX, posY)
  w = *image\width
  h = *image\height
  channel = *image\nChannels
  wMask = *mask\width
  ...
  *gray1.IplImage = cvCreateImage(wMask, hMask, #IPL_DEPTH_8U, 1)
  *gray2.IplImage = cvCreateImage(wMask, hMask, #IPL_DEPTH_8U, 3)
  cvCvtColor(*mask, *gray1, #CV_BGR2GRAY, 1)
  cvMerge(*gray1, *gray1, *gray1, #Null, *gray2) 
  GetGradientX(*gray2, *sgx)
  GetGradientY(*gray2, *sgy)
  ...
  *fy.IplImage = cvCreateImage(w, h, #IPL_DEPTH_32F, 3)
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      For c = 0 To channel - 1
        PokeF(@CV_IMAGE_ELEM(*fx, i, (j * channel + c) * 4), PeekF(@CV_IMAGE_ELEM(*grx32, i, (j * channel + c) * 4)) + PeekF(@CV_IMAGE_ELEM(*srx32, i, (j * channel + c) * 4)))
        PokeF(@CV_IMAGE_ELEM(*fy, i, (j * channel + c) * 4), PeekF(@CV_IMAGE_ELEM(*gry32, i, (j * channel + c) * 4)) + PeekF(@CV_IMAGE_ELEM(*sry32, i, (j * channel + c) * 4)))
      Next
    Next
  Next
  ...
  *r_result.CvMat = cvCreateMat(h, w, CV_MAKETYPE(#CV_8U, 1))
  ...
  If keyPressed = 32
    Repeat
      ShowImage(*image, *mask, posX, posY)
      cvShowImage(#CV_WINDOW_NAME, *image)
      keyPressed = cvWaitKey(0)
    Until keyPressed = 13 Or keyPressed = 27 Or keyPressed = 32 Or exitCV
  ...
  If keyPressed = 13 : exitCV = 255 : EndIf
...
ProcedureC OpenCV(ImageFile.s, nBlend)
  If FileSize(ImageFile) > 0
    ...
    *temp.IplImage = cvLoadImage("images/blend3_" + Str(nBlend + 1) + "b.jpg", #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)

    Select nBlend
      Case 0
        *roi.IplImage = cvCreateImage(100, 100, #IPL_DEPTH_8U, *temp\nChannels)
        cvSetImageROI(*temp, 50, 120, 100, 100)
      Case 1
        *roi.IplImage = cvCreateImage(*resize\width - 20, *resize\height - 20, #IPL_DEPTH_8U, *temp\nChannels)
        cvSetImageROI(*temp, 10, 10, *resize\width - 20, *resize\height - 20)
      Case 2
        *roi.IplImage = cvCreateImage(*temp\width, *resize\height - 5, #IPL_DEPTH_8U, *temp\nChannels)
        cvSetImageROI(*temp, 0, 0, *temp\width, *resize\height - 5)
    EndSelect
    ...
    If *resize\nChannels = 3 And *roi\nChannels = 3
      ...
      *param\uValue = window_handle
      ...
      Select nBlend
        Case 0
          x = (*resize\width / 2) - (*roi\width / 2) - 20
          y = (*resize\height / 2) - (*roi\height / 2) + 20
        Case 1
          x = 10
          y = 10
        Case 2
          x = (*resize\width / 2) - (*roi\width / 2)
          y = 5
      EndSelect
      *blend.IplImage = PoissonBlend(*resize, *roi, x, y)
      ...
      FreeMemory(*param)
    Else
      MessageBox_(0, "Process Cancelled..." + Chr(10) + Chr(10) + "One of the images does not meet the channel requirements.", #CV_WINDOW_NAME, #MB_ICONERROR)
    EndIf
    cvReleaseImage(@*roi)
    cvReleaseImage(@*temp)
    ...
    If keyPressed = 13
      exitCV = #False
      nBlend = (nBlend + 1) % 3
      OpenCV("images/blend3_" + Str(nBlend + 1) + "a.jpg", nBlend)
    EndIf
  EndIf
EndProcedure

ExamineDesktops()
OpenCV("images/blend3_1a.jpg", 0)
