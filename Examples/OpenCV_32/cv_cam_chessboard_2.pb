...
#CV_DESCRIPTION = "Finds the positions of internal corners for a 10 x 7 chessboard pattern.  First calibrating " +
                  "the webcam, then drawing a transparent cube in 3D space." + Chr(10) + Chr(10) +
  ...
  *param\uValue = window_handle
  ...
  If found_num = #IMAGE_SIZE
    ...
    *translation_vectors.CvMat = cvCreateMat(#IMAGE_SIZE, 3, CV_MAKETYPE(#CV_32F, 1))
    cvCalibrateCamera2(@object_points, @image_points, @point_counts, FrameWidth, FrameHeight, *camera_matrix, *distortion_coeffs, *rotation_vectors, *translation_vectors, 0, #CV_TERMCRIT_ITER | #CV_TERMCRIT_EPS, 30, DBL_EPSILON)
        ...
        *source = cvCloneImage(*image)
        ...
        If cvCheckChessboard(*gray, #CORNER_COL, #CORNER_ROW)
          ...
          If found
            ...
            cvInitMatHeader(@object_points, #PATTERN_SIZE, 3, CV_MAKETYPE(#CV_32F, 1), @objects_forLoop(), #CV_AUTOSTEP)
            cvInitMatHeader(@image_points, #PATTERN_SIZE, 2, CV_MAKETYPE(#CV_32F, 1), @corners_forLoop(), #CV_AUTOSTEP)
            cvFindExtrinsicCameraParams2(@object_points, @image_points, *camera_matrix, *distortion_coeffs, *rotation_vectors, *translation_vectors, 0)
