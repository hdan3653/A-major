...
Global getCV.b, *save.IplImage, exitCV.b, lpPrevWndFunc, nDepth

#CV_WINDOW_NAME = "PureBasic Interface to OpenCV"
#CV_DESCRIPTION = "Smooth the edges of an image using a normalized convolution filter." + Chr(10) + Chr(10) +
...
ProcedureC NormalizedFilter(*smooth.CvMat, *horiz.CvMat, radius.f)
  myinf.d = 9.9e307
  h = *smooth\rows
  w = *smooth\cols
  channel = (CV_MAT_TYPE(*smooth\type) / 8) + 1
...
ProcedureC SmoothEdge(*image.IplImage, sigma_s.f, sigma_r.f)
  ...
  *smooth_t.CvMat = cvCreateMat(w, h, CV_MAKETYPE(#CV_32F, 3))
  ...
  NormalizedFilter(*smooth, *ctH, radius)
  cvTranspose(*smooth, *smooth_t)
  NormalizedFilter(*smooth_t, *ctV_t, radius)
    ...
    If *resize\nChannels = 3
      ...
      *param\uValue = window_handle
      ...
      If keyPressed = 13
        exitCV = #False
        nDepth = (nDepth + 1) % 2
        OpenCV("images/smooth" + Str(nDepth + 1) + ".jpg")
      ElseIf getCV
        getCV = #False
        exitCV = #False
        OpenCV(GetImage())
