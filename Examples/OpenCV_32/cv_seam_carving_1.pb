...
#CV_DESCRIPTION = "Deleting vertical seams in an image by calculating the " +
                  "intensity of color pixels (experimental)." + Chr(10) + Chr(10) +
  ...
  If FileSize(ImageFile) > 0
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200 And *resize\nChannels = 3
      ...
      *edge.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_8U, 3)
      nAperture = 1
      BringToTop(hWnd)
      ...
      *param\uValue = window_handle
      ...
      Repeat
        If sobel
          Select sobel
            Case 1
              cvSobel(*resize, *edge, 1, 1, nAperture)
            Case 2
              cvSobel(*resize, *edge, 1, 0, nAperture)
            Case 3
              cvSobel(*resize, *edge, 0, 1, nAperture)
          EndSelect
          cvShowImage(#CV_WINDOW_NAME + " - Seam Carving", *edge)
        Else
          cvSobel(*resize, *edge, 1, 1, 5)
      ...
      If keyPressed = 32
        ...
        *clone.IplImage = cvCloneImage(*resize)
        ...
        For i = *resize\width - 1 To 0 Step -2
          ...
          For j = 0 To *resize\height - 1
            ...
            Else
              ...
              If B = 0 And G = 255 And R = 0
                intensity_1 = 1000
              Else
                B = PeekA(@*edge\imageData\b + (j * *edge\widthStep) + (iPosition + 1) * 3 + 0)
                G = PeekA(@*edge\imageData\b + (j * *edge\widthStep) + (iPosition + 1) * 3 + 1)
                R = PeekA(@*edge\imageData\b + (j * *edge\widthStep) + (iPosition + 1) * 3 + 2)
                intensity_1 = B + G + R
              EndIf
            EndIf
            ...
            If B = 0 And G = 255 And R = 0
              intensity_2 = 1000
            Else
              B = PeekA(@*edge\imageData\b + (j * *edge\widthStep) + iPosition * 3 + 0)
              G = PeekA(@*edge\imageData\b + (j * *edge\widthStep) + iPosition * 3 + 1)
              R = PeekA(@*edge\imageData\b + (j * *edge\widthStep) + iPosition * 3 + 2)
              intensity_2 = B + G + R
            EndIf
          ...
          *edge = cvCreateImage(*seams\width, *seams\height, #IPL_DEPTH_8U, 3)
          *clone = cvCloneImage(*seams)

          Select sobel
            Case 0
              cvSobel(*seams, *edge, 1, 1, 5)
            Case 1
              cvSobel(*seams, *edge, 1, 1, nAperture)
            Case 2
              cvSobel(*seams, *edge, 1, 0, nAperture)
            Case 3
              cvSobel(*seams, *edge, 0, 1, nAperture)
          EndSelect
