...
#CV_DESCRIPTION = "Simulates the LOMO (Leningrad Optical Mechanical Association) camera lens effect." + Chr(10) + Chr(10) +
                  "- SPACEBAR: Toggle LOMO effects." + Chr(10) + Chr(10) +
                  "- ENTER: Reset the image."
...
ProcedureC SetHSV(*image.IplImage, h, s, v)
...
ProcedureC SetBrightnessAndContrast(*image.IplImage, Brightness, Contrast)
  Dim lut.LUT_VALUES(256)
  *lut_mat.CvMat = cvCreateMatHeader(1, 256, CV_MAKETYPE(#CV_8U, 3))
  cvSetData(*lut_mat, lut(), 0)
...
ProcedureC SetGammaCurves(*image.IplImage, rIn, rOut, gIn, gOut, bIn, bOut)
  Dim lut.LUT_VALUES(256)
  rr = Log(rOut) / Log(rIn)
  rg = Log(gOut) / Log(gIn)
  rb = Log(bOut) / Log(bIn)
...
ProcedureC NaturalVignetting(imageWidth, imageHeight, maxAngle.f)
...
ProcedureC MechanicalVignetting(imageWidth, imageHeight, fallOff)
...
ProcedureC ArtificialVignetting(*bgr.IplImage, maxAngle.f, vStyle, vSize)
...
ProcedureC ProcessVignetting(*image.IplImage, degrees, vStyle, vSize)
