...
#CV_DESCRIPTION = "Calculates the contour areas." + Chr(10) + Chr(10) + "For each point set:" + Chr(10) +
                  "- degree of tilt / center of mass" + Chr(10) + "- bounding ellipse / minimal area"
      ...
      *contour.IplImage = cvCreateImage(*gray\width, *gray\height, #IPL_DEPTH_8U, 3)
      ...
      For rtnCount = 0 To nContours - 1
        ...
        If area >= 35 And area <= 100000
          Dim pts1.CvPoint(*contours\total)
          Dim pts2.CvPoint2D32f(*contours\total)
          cvDrawContours(*contour, *contours, 255, 155, 0, 0, 155, 255, 0, 0, 0, 1, #CV_AA, 0, 0)
          cvCvtSeqToArray(*contours, @pts1(), 0, #CV_WHOLE_SEQ_END_INDEX)

          For rtnPoint = 0 To *contours\total - 1
            pts2(rtnPoint)\x = pts1(rtnPoint)\x
            pts2(rtnPoint)\y = pts1(rtnPoint)\y
          Next
          cvFitEllipse(pts2(), *contours\total, @box)
          cvEllipse(*contour, box\center\x, box\center\y, box\size\width * 0.65, box\size\height * 0.65, box\angle, 0, 360, 0, 255, 255, 0, 1, #CV_AA, #Null)
