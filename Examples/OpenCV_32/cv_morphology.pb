...
#CV_DESCRIPTION = "Performs advanced morphological transformations using a specific structuring element." + Chr(10) + Chr(10) +
                  "- TRACKBAR: Adjust the iterations." + Chr(10) + Chr(10) +
                  "- SPACEBAR: Change the element." + Chr(10) + Chr(10) +
                  "- [ O ] KEY: Morphological operations."
  ...
  If FileSize(ImageFile) > 0
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200
      cvCreateTrackbar("Iterations", #CV_WINDOW_NAME, 0, 21, @CvTrackbarCallback())
      ...
      Repeat
        If *resize
          *kernel = cvCreateStructuringElementEx(nPosition * 2 + 1, nPosition * 2 + 1, nPosition, nPosition, shape, #Null)
          cvMorphologyEx(*reset, *resize, #Null, *kernel, operation, 1)
          ...
          Select keypressed
            Case 32
              shape = (shape + 1) % 3
            Case 79, 111
              If nPosition : operation = ((operation + 1) % 5) + 2 : Else : operation = ((operation + 1) % 2) + 2 : EndIf
          EndSelect
        EndIf
