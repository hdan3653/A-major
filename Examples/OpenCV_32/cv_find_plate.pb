...
#CV_DESCRIPTION = "Number plate recognition (experimental)."
    ...
    For rtnCount = 0 To nContours - 1
      ...
      If area >= 1000 And area <= 7000
        ...
        If cAspect > minAspect And cAspect < maxAspect And percPixels < 0.8 And bHeight >= minHeight And bHeight < maxHeight
          cvBoxPoints(box\center\x, box\center\y, box\size\width, box\size\height, box\angle, pts())

          For pCount = 0 To 4 - 1
            cvLine(*contour, pts(pCount)\x, pts(pCount)\y, pts((pCount + 1) % 4)\x, pts((pCount + 1) % 4)\y, 0, 255, 255, 0, 2, #CV_AA, #Null)
          Next
    ...
    For nBox = 0 To nCount - 1
      ...
      *seq = cvCreateSeq(#CV_SEQ_ELTYPE_POINT, SizeOf(CvSeq), SizeOf(CvPoint), *storage)
      ...
      If cAspect > minAspect And cAspect < maxAspect And bHeight >= minHeight And bHeight < maxHeight
        cvBoxPoints(minRect\center\x, minRect\center\y, minRect\size\width, minRect\size\height, minRect\angle, pts())

        For pCount = 0 To 4 - 1
          cvLine(*resize1, pts(pCount)\x, pts(pCount)\y, pts((pCount + 1) % 4)\x, pts((pCount + 1) % 4)\y, 0, 255, 255, 0, 2, #CV_AA, #Null)
        Next
        ...
        *threshold2 = cvCreateImage(*resize2\width, *resize2\height, #IPL_DEPTH_8U, 1)
        ...
        For rtnCount = 0 To nContours - 1
          cvBoundingRect(@rect, *contours, 1)
          cvRectangleR(*resize2, rect\x - 1, rect\y - 1, rect\width + 2, rect\height + 2, 70, 70, 255, 0, 1, #CV_AA, #Null)
          *contours = *contours\h_next
        Next
