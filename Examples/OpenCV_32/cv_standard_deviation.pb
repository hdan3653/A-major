...
#CV_DESCRIPTION = "Applies Standard Deviation to an image by calculating the square root of the variance." +
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200 And *resize\nChannels = 3
      *gray.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_8U, 1)
      ...
      temp.CvMat : *temp.CvMat = cvGetMat(*gray, @temp, #Null, 0)
      *mat.CvMat = cvCreateMat(*temp\rows, *temp\cols, CV_MAKETYPE(#CV_32F, 1))
      *multiply1.CvMat = cvCreateMat(*mat\rows, *mat\cols, CV_MAKETYPE(#CV_32F, 1))
      *multiply2.CvMat = cvCreateMat(*mat\rows, *mat\cols, CV_MAKETYPE(#CV_32F, 1))
      *sigma.CvMat = cvCreateMat(*mat\rows, *mat\cols, CV_MAKETYPE(#CV_32F, 1))
      *scale.CvMat = cvCreateMat(*mat\rows, *mat\cols, CV_MAKETYPE(#CV_8U, 1))
      *color.IplImage = cvCreateImage(*resize\width, *resize\height, #IPL_DEPTH_8U, 3)
      cvConvertScale(*temp, *mat, 1 / 255, 0)
      cvSmooth(*mat, *multiply1, #CV_BLUR, 3, 3, 0, 0)
      cvMul(*mat, *mat, *mat, 1)
      cvSmooth(*mat, *multiply2, #CV_BLUR, 3, 3, 0, 0)
      cvMul(*multiply1, *multiply1, *multiply1, 1)
      cvSub(*multiply2, *multiply1, *multiply2, #Null)
      cvPow(*multiply2, *sigma, 0.5)
      cvNormalize(*sigma, *sigma, 0, 1, #NORM_MINMAX, #Null)
      cvConvertScale(*sigma, *scale, 255, 0)
      cvCvtColor(*scale, *color, #CV_GRAY2BGR, 1)
      ...
      *param\uValue = window_handle
      ...
      cvReleaseMat(@*scale)
      cvReleaseMat(@*sigma)
      cvReleaseMat(@*multiply2)
      cvReleaseMat(@*multiply1)
      cvReleaseMat(@*mat)
      cvReleaseImage(@*gray)
