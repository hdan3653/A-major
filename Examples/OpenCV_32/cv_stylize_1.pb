...
Global getCV.b, *save.IplImage, exitCV.b, lpPrevWndFunc, nDepth, nStyle

#CV_WINDOW_NAME = "PureBasic Interface to OpenCV"
#CV_DESCRIPTION = "Stylize an image by applying various filters." + Chr(10) + Chr(10) +
                  "- SPACEBAR: Start the stylizing." + Chr(10) + Chr(10) +
...
ProcedureC NormalizedFilter(*style.CvMat, *horiz.CvMat, radius.f)
  myinf.d = 9.9e307
  h = *style\rows
  w = *style\cols
  channel = (CV_MAT_TYPE(*style\type) / 8) + 1
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      cvGet2D(@scalar1, *final, i, j)
	    cvSet2D(*style, i, j, scalar1\val[0], scalar1\val[1], scalar1\val[2], 0)
    Next
  Next
  cvReleaseMat(@*flag)
  cvReleaseMat(@*b)
...
ProcedureC SmoothEdge(*image.IplImage, sigma_s.f, sigma_r.f)
  ...
  *style.CvMat = cvCreateMat(h, w, CV_MAKETYPE(#CV_32F, 3))
  *style_t.CvMat = cvCreateMat(w, h, CV_MAKETYPE(#CV_32F, 3))
  ...
  For i = 0 To h - 1
    For j = 0 To w - 1
      ...
      cvSet2D(*style, i, j, val0, val1, val2, 0)
    Next
  Next
  radius.f = Sqr(3) * sigma_s * Sqr(3) / Sqr(Pow(4, 1) - 1)
  NormalizedFilter(*style, *ctH, radius)
  cvTranspose(*style, *style_t)
  NormalizedFilter(*style_t, *ctV_t, radius)
  cvTranspose(*style_t, *style)
  cvReleaseMat(@*style_t)
  cvReleaseMat(@*ctV_t)
  cvReleaseMat(@*ctV)
  cvReleaseMat(@*ctH)
	cvReleaseMat(@*vert)
    ...
    If *resize\nChannels = 3
      ...
      *param\uValue = window_handle
      ...
      If keyPressed = 32
        ...
        *resize32.IplImage = cvCreateImage(w, h, #IPL_DEPTH_32F, 3)
        ...
        cvWaitKey(100)
    		sigma_s.f = 60
    		sigma_r.f = 0.30
    		*style.CvMat = SmoothEdge(*resize32, sigma_s, sigma_r)
    		Dim *planes.CvMat(3)

    		For rtnCount = 0 To 3 - 1
    		  *planes(rtnCount) = cvCreateMat(h, w, CV_MAKETYPE(#CV_32F, 1))
    		Next
    		cvSplit(*style, *planes(0), *planes(1), *planes(2), 0)
        ...
        *magYB.CvMat = cvCreateMat(h, w, CV_MAKETYPE(#CV_32F, 1))
        cvSobel(*planes(0), *magXR, 1, 0, 3)
        cvSobel(*planes(0), *magYR, 0, 1, 3)
        cvSobel(*planes(1), *magXG, 1, 0, 3)
        cvSobel(*planes(1), *magYG, 0, 1, 3)
        cvSobel(*planes(2), *magXB, 1, 0, 3)
        cvSobel(*planes(2), *magYB, 0, 1, 3)
        ...
        *magnitude.CvMat = cvCreateMat(h, w, CV_MAKETYPE(#CV_32F, 1))
        ...
        Next
          ...
          Next
        Next
        *stylized.CvMat = cvCreateMat(h, w, CV_MAKETYPE(#CV_32F, 3))
        scalar.CvScalar

        For i = 0 To h - 1
          For j = 0 To w - 1
            cvGet2D(@scalar, *style, i, j)
            magnitude.f = cvmGet(*magnitude, i, j)
            val0.f = scalar\val[0] * magnitude
            val1.f = scalar\val[1] * magnitude
            val2.f = scalar\val[2] * magnitude
            cvSet2D(*stylized, i, j, val0, val1, val2, 0)
          Next
        Next
        *temp.IplImage = cvCreateImage(w, h, #IPL_DEPTH_32F, 3)
        *final.IplImage = cvCreateImage(w, h, #IPL_DEPTH_8U, 3)
        scalar.CvScalar

        For i = 0 To h - 1
          For j = 0 To w - 1
            cvGet2D(@scalar, *stylized, i, j)
            PokeF(@CV_IMAGE_ELEM(*temp, i, (j * 3 + 0) * 4), scalar\val[0])
            PokeF(@CV_IMAGE_ELEM(*temp, i, (j * 3 + 1) * 4), scalar\val[1])
            PokeF(@CV_IMAGE_ELEM(*temp, i, (j * 3 + 2) * 4), scalar\val[2])
          Next
        Next
      ...
      If keyPressed = 13
        exitCV = #False
        nStyle = (nStyle + 1) % 2
        OpenCV("images/style" + Str(nStyle + 1) + ".jpg")
      ElseIf getCV
        getCV = #False
        exitCV = #False
        OpenCV(GetImage())
