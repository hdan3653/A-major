...
#CV_DESCRIPTION = "Calculates a feature map for corner detection." + Chr(10) + Chr(10) + "- TRACKBAR: Adjust the detection."
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200
      ...
      *map.CvMat = cvCreateMat(*resize\height, *resize\width, CV_MAKETYPE(#CV_32F, 1))
      cvPreCornerDetect(*gray, *map, 3)
      corner.CvScalar
      nCorner = 0.01
      *param.USER_INFO = AllocateMemory(SizeOf(USER_INFO))
      *param\uPointer1 = *corner
      *param\uValue = window_handle
      cvSetMouseCallback(*window_name, @CvMouseCallback(), *param)

      Repeat
        If *resize
          *corner = cvCloneImage(*resize)

          For y = 0 To *resize\height - 1
            For x = 0 To *resize\width - 1
              cvGet2D(@corner, *map, y, x)

              If corner\val[0] > nCorner : cvCircle(*corner, x, y, 2, 0, 0, 255, 0, #CV_FILLED, #CV_AA, #Null) : EndIf

            Next
          Next
          cvShowImage(#CV_WINDOW_NAME, *corner)
