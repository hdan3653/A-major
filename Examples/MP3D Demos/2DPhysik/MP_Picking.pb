...
Global Event
;- ***  Create Window
MP_Graphics3DWindow(0, 0, wX, wY, "Chipmunk4MP3D Demo: Picking Demo, left Mouse picking, Space makes more Objects", #PB_Window_ScreenCentered | #PB_Window_SystemMenu)
...
If CreateImage(0, 256, 256,32)
  MP_CreateImageColored(0, 0, #Red,#Green,#Green,#Red)     
...
*Memory = AllocateMemory(4*SizeOf(CMunkVect) )
...
For i=0 To 8
   For j=0 To 5
       ...
       *Body = MP_2DPhysicStaticCircle(i*80+stagger,100+(j*70), 15, 0, RGB(Random(255),Random(255),Random(255)))
...
Repeat
  ...
  If switch = 1
      MP_2DPhysicBodySetColor(*shape,  RGB(Random(255),Random(255),Random(255)))
...
;}
  ...
  If MP_KeyDown( #PB_Key_Space)
    ;{ Make Body
    AddElement (Polys())
    Polys() = MP_2DPhysicBodyBox(wX/2, -20, 20, 20, Sprite , 1)    
    obj +1
    ;}
