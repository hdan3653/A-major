...
; 1: Í∞Ä?ÖÎ†• 2: ?ÖÎ†• 3: ?ÖÎ≥¥ ?¨ÏÉù?òÍ∏∞
; Ï≤òÏùå ?úÏûë ?? ÎßàÏö∞??Ïª§ÏÑú?Ä ?§Î≥¥??1(?πÏ? 2)Î°?Î∞ïÏä§ ?ÅÏó≠ ?§Ï†ï -> ?§Ìéò?¥Ïä§Î∞îÎ°ú ?ÅÌÉú ?ÑÌôò -> ÎßàÏö∞??Ïª§ÏÑú?Ä ?§Î≥¥??1(?πÏ? 2)Î°???Ï∂úÎ†•
...
Structure mySprite
  ...
  x.i   ; ?ÑÏπò x
  y.i   ; ?ÑÏπò y
  width.i   ; ?ÑÏ≤¥ Í∞ÄÎ°??¨Ïù¥Ï¶?
  height.i  ; ?ÑÏ≤¥ ?∏Î°ú ?¨Ïù¥Ï¶?
  
  present.i; ?ÑÏû¨ ?ÑÎ†à??
  frametime.i
  
  f_width.i ; ???ÑÎ†à?ÑÏùò Í∞ÄÎ°??¨Ïù¥Ï¶?
  f_height.i; ???ÑÎ†à?ÑÏùò ?∏Î°ú ?¨Ïù¥Ï¶?
  f_horizontal.i   ; Í∞ÄÎ°??ÑÎ†à????
  f_vertical.i     ; ?∏Î°ú ?ÑÎ†à????
...
Procedure InitMySprite(name.s, filename.s, x.i, y.i, active.i = 1) ;active???µÏÖò
                                                                   ; ?§ÌîÑ?ºÏù¥??Íµ¨Ï°∞Ï≤?Ï¥àÍ∏∞??
...
;?§ÌîÑ?ºÏù¥??Ï¢åÌëúÍ∞? ?úÏÑ±?îÏó¨Î∂Ä Î≥ÄÍ≤?
...
; sprite_list ?êÏÑú ?¥Î¶Ñ?ºÎ°ú Íµ¨Ï°∞Ï≤?Ï∞æÍ∏∞. ?®Î≤† ?πÏÑ±??current element ?¥Ïäà ?åÎ¨∏???ÑÏ§ë???ºÏπò?¥ÎèÑ ?ùÍπåÏßÄ Î£®ÌîÑÎ•??åÏïÑ?ºÌï®
...
Procedure CalcCodeArea_Lv2(x, y)
  ...
  ProcedureReturn code ; ÏΩîÎìúÎ•?Î∞òÌôò
EndProcedure
...
Procedure MovingAnt_Lv2(code.i, bar.i)
  ...
  *n2 = FindSprite("note"+Str(currentBar)+"\2")
  ...
  Repeat
    ...
    currentTime = GetTickCount_()
    ForEach sprite_list()
      FrameManager(sprite_list()) ;active ?ÅÌÉú??Í≤ÉÎì§Îß??§Ïùå ?ÑÎ†à?ÑÏúºÎ°?
    Next
    ForEach sprite_list()
...
Procedure DrawNotes_Lv2()
  ...
  For b=currentBar To MaxBar-1
    ...
    ;       ; ?îÎ©¥???òÏπòÍ≤?Í∑∏Î†§ÏßÄÎ©??îÏù¥??Í∑∏Î¶¨ÏßÄ ?äÏùå
    ;       Break
    ;     EndIf
    ...
    For i=0 To bar\noteCount-1
      ...
      ; ?§Ïùå ?åÌëúÎ•??ÑÌïú Í±∞Î¶¨ Í∞ÄÏ§ëÏπò (Í∞ÑÍ≤©??ÎßûÏ∂∞Ï£ºÍ∏∞ ?ÑÌï¥)
...
Procedure AnswerCheck_Lv2()
  For i=0 To MaxBar-1
    If problem_list(currentProblem)\fixed(i)
      ; Ï£ºÏñ¥Ïß??µÏ? ÎπÑÍµê?òÏ? ?äÏùå
      Continue
    EndIf
    answer = problem_list(currentProblem)\answers(i)
    input = userAnswers(i)
    If answer = input
      PrintN(Str(i+1) + "Î≤àÏß∏ ÎßàÎîî ?ïÎãµ / ?ÖÎ†• : " + Str(userAnswers(i)) + "/ ?§Ï†ú ??: " + Str(answer))
    Else
      PrintN(Str(i+1) + "Î≤àÏß∏ ÎßàÎîî ?§Îãµ / ?ÖÎ†• : " + Str(userAnswers(i)) + "/ ?§Ï†ú ??: " + Str(answer))
    EndIf
  Next
EndProcedure
...
Procedure PlayNotes(parameter)
  ...
  For i=0 To 7
    ...
    For j=0 To noteCount-1
      ...
      ; ?ÑÏπò Î≥ÄÍ≤?
      dist.f = -80*weight.f
      If j=0 And i<>0 ; Ï≤??åÏù¥Î©¥ÏÑú Ï≤?ÎßàÎîîÍ∞Ä ?ÑÎãò
                      ; separator ÎßåÌÅº ???ÄÏßÅÏó¨????
        dist.f + (-80*2)
      EndIf
...
markerState = 0 ; ÎßàÏª§ ?ÖÎ†• ?ÅÌÉú
threadStatus = 0; thread ?ÅÌÉú. 0-?§Ìñâ?àÌï®, 1-?§ÌñâÏ§?
...
If *capture
  ...
  *loadbox2 = cvLoadImage("../graphics/chord_box2.png", 1)
  ...
  If OpenWindow(0, 0, 0, FrameWidth, FrameHeight, "PureBasic Interface to OpenCV", #PB_Window_SystemMenu |#PB_Window_MaximizeGadget | #PB_Window_ScreenCentered|#PB_Window_Maximize)
    ...
    Window_1=OpenWindow(1, 0, WindowHeight(0)/2 - 200, FrameWidth-5, FrameHeight-30, "title") ; ?πÏ∫†???àÎèÑ??
    ImageGadget(0, 0, 0, FrameWidth, FrameHeight, ImageID(pbImage))
    StickyWindow(1, #True) ; ??ÉÅ ?ÑÏóê Í≥†Ï†ï
    SetWindowLongPtr_(WindowID(1), #GWL_STYLE, GetWindowLongPtr_(WindowID(0), #GWL_STYLE)&~ #WS_THICKFRAME &~ #WS_DLGFRAME) ; ?àÎèÑ???Ä?¥Ì? Î∞??úÍ±∞
