...
Procedure CheckArea_Tutorial(bit$)
  If(bit$ = "1")
  ...
  Select Tutorial_Num
    ...
    Case 10
      *p = FindSprite("highlight2")
      ...
      WriteScript("마커로 음을 입력하기 위해 왼쪽마커의 화살표를 아래로한 뒤 B버튼을 눌러"+#CRLF$+"마커 모드를 음 입력모드로 바꿀 수 있습니다.", scriptPos_x, scriptPos_y) 
      Tuto_Lock_10= #True
          ...
          *image = cvQueryFrame(*capture)
          ...
          If *image
            ...
            If (Asc(micbit$) > 47 And Asc(micbit$) < 50)
              ...
              If markerState = 1
                If  Tuto_Lock_15 = #False 
                  CheckArea_Tutorial(micbit$)
                ElseIf    Tuto_Lock_15 
                  CheckArea_Lv2(micbit$)
                EndIf 
              EndIf
            Else
              If(micbit$ = "5")
            ...
            *mat.CvMat = cvEncodeImage(".bmp", *image, 0)     
            ...
            If  micbit$ = "7" And Tutorial_State = #Tutorial_GameInPlay;Escape
