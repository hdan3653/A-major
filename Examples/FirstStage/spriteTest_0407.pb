...
Global Dim elements(2) ; container 안에 3개 element
...
Procedure InitProblem()
  ...
  For k = 0 To 3
    Print("currentProblem(k): ")
    PrintN(Str(currentProblem(k)))
  Next
...
Procedure DropNote()  
  ...
  *container.mySprite = FindSprite("container")
  ...
  If stageNum = 1  
    *note1.mySprite = FindSprite("bubble" + Str(chord_list(currentProblem(0), currentProblem(1))))
    *note2.mySprite = FindSprite("bubble" + Str(chord_list(currentProblem(0), currentProblem(2)))) 
  ...
  ElseIf stageNum = 2
    *note1.mySprite = FindSprite("bubble" + Str(chord_list(currentProblem(0), currentProblem(1))))
    ...
    If answerNum = 1
      elements(1) = *bubble
    ElseIf answerNum = 2
      *note2.mySprite = FindSprite("answer1")
      InitMyPosition(*note2, 10, 0, line_position(answerTone) + 50, 0, 20)
      elements(2) = *bubble
    EndIf 
  ElseIf stageNum = 3
    If answerNum = 1
      elements(0) = *bubble
    ElseIf answerNum = 2
      *note1.mySprite = FindSprite("answer1")
      InitMyPosition(*note1, 10, 0, line_position(answerTone), 0, 20)
      elements(1) = *bubble
    ElseIf answerNum = 3
      *note1.mySprite = FindSprite("answer1")
      InitMyPosition(*note1, 10, 0, line_position(answerTone), 0, 20)
      *note2.mySprite = FindSprite("answer2")
      InitMyPosition(*note2, 10, 0, line_position(answerTone) + 50, 0, 20)
      elements(2) = *bubble
    EndIf 
  EndIf 
...
;If *capture
  FrameWidth = 640;cvGetCaptureProperty(*capture, #CV_CAP_PROP_FRAME_WIDTH)
  FrameHeight = 480;cvGetCaptureProperty(*capture, #CV_CAP_PROP_FRAME_HEIGHT)
