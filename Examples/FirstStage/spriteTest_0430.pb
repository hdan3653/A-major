...
Procedure DrawBoxs(*image)
  ...
  If direction = 0   
    width = ptBox(6, 2)\x - ptBox(0, 0)\x
    height = ptBox(0, 2)\y - ptBox(0, 0)\y
  Else
    width = ptBox(0, 2)\x - ptBox(0, 0)\x
    height = ptBox(0, 2)\y - ptBox(6, 0)\y
  EndIf
  ...
  Debug Str(ptBox(6,0)\x) + "    " + Str(ptBox(6,0)\y)
  ...
  *boximg.IplImage = cvCreateImage(width, height, #IPL_DEPTH_8U, 3)
  ...
  Else
    ...
    cvSetImageROI(*image, ptBox(6, 0)\x, ptBox(6, 0)\y, width, height)
  EndIf
    ...
    *p\f_height = *p\height
    ...
    Repeat
      ...
      *mat.CvMat = cvEncodeImage(".bmp", *image, 0)
      ;*mat.CvMat = cvEncodeImage(".bmp", *loadbox, 0)     
